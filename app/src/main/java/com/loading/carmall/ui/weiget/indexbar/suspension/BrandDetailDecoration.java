 ...
 * 待完成：
 */
public class BrandDetailDecoration extends RecyclerView.ItemDecoration {
    ...
    public BrandDetailDecoration(Context context, List<? extends ISuspensionInterface> datas) {
        super();
        mContext = context;
        if (null == datas)
            datas = new ArrayList<>();
    ...
    public BrandDetailDecoration setmTitleHeight(int mTitleHeight) {
        this.mTitleHeight = mTitleHeight;
        return this;
    }


    public BrandDetailDecoration setColorTitleBg(int colorTitleBg) {
        COLOR_TITLE_BG = colorTitleBg;
        return this;
    }

    public BrandDetailDecoration setColorTitleFont(int colorTitleFont) {
        COLOR_TITLE_FONT = colorTitleFont;
        return this;
    }

    public BrandDetailDecoration setTitleFontSize(int mTitleFontSize) {
        mPaint.setTextSize(mTitleFontSize);
        return this;
    }

    public BrandDetailDecoration setmDatas(List<? extends ISuspensionInterface> mDatas) {
        this.mDatas = mDatas;
        return this;
    }
    ...
    public BrandDetailDecoration setHeaderViewCount(int headerViewCount) {
    ...
    public void onDraw(Canvas c, RecyclerView parent, RecyclerView.State state) {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (mDatas == null || mDatas.isEmpty() || position > mDatas.size() - 1
                    || position < 0 || !mDatas.get(position).isShowSuspension()) {
                continue;//越界
            }
            ...
            if (position > -1) {
                if (position == 0) {//等于0肯定要有title的
                    ...
                    if (null != mDatas.get(position).getSuspensionTag() &&
                            !mDatas.get(position).getSuspensionTag().equals(mDatas.get(position - 1)
                                    .getSuspensionTag())) {
...
/*
        ...
        // TODO: 2017/5/15 天津4s专营店
        mPaint.getTextBounds("天津4s专营店", 0, "天津4s专营店".length(), mBounds);
...
//        c.drawText(mDatas.get(position).getSuspensionTag(), child.getPaddingLeft() + marginLeft, child.getTop() - params.topMargin - (mTitleHeight / 2 - mBounds.height() / 2), mPaint);
        c.drawText("天津4s专营店", child.getPaddingLeft() + marginLeft, child.getTop() - params.topMargin - (mTitleHeight / 2 - mBounds.height() / 2), mPaint);
    }
...
//                Log.d("zxt", "onDrawOver() called with: c = [" + child.getTop());//当getTop开始变负，它的绝对值，是第一个可见的Item移出屏幕的距离，
        ...
        // TODO: 2017/4/8 设置首页悬停索引标题位置为26dp
        int marginLeft = ConvertUtils.dp2px(16);
